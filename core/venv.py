...
class VenvHandler():
    ...
    def create_venv (self, name):
        venv_name = name if name else input("\033[92mInput virtual environment name: \033[0m")
        ...
        try:
            ...
            logger.info(f"Created {venv_name} successfully in {VENV_DIR}")
            print("  Venv created successfully!\n")
        except subprocess.CalledProcessError as e:
            logger.error(f"Error creating virtual environment: {e}")
            logger.exception("Unexpected crash encountered. Traceback: ")
