...
- The **listener agent** accepts unauthenticated UDP commands (`SHUTDOWN`, `RESTART`, `LOGOUT`, `KILL`) from anyone on the network who knows the port — there's a `secret` field reserved in `settings.json` under `LISTENER`, but it isn't currently checked against incoming messages. Consider enforcing it, and/or restricting the firewall rule (`config.intiate_listener()`) to trusted subnets.
