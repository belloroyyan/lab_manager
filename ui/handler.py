...
def handle_sort():
    print(f"{YELLOW}Detected drives:\n{RESET}")
    drives = get_drives()
    i = 0
    for d in drives:
...
======================================================================
  ...
  [0] Back to Main Menu""")
    ...
    if choice == "1":
        path = input(f"\n{GREEN}Enter path to analyze (leave blank for user folder): {RESET}").strip()
        ...
        path = input(f"\n{GREEN}Enter path to scan for large files: {RESET}").strip()
        size = input(f"\n{GREEN}Minimum size in MB to move (default 100): {RESET}").strip()
...
def handle_settings():
  ...
  [0] Back to Main Menu""")
    ...
    try:
        if choice[0:4] == "edit":
            section_ = input(f"\n{GREEN}Input setting (section_name): {RESET}").strip().upper()
            section = config.get(section_)
            display_all_settings(config[section_], 3)
            key_ = input(f"\n{GREEN}Input setting (key): {RESET}").strip().lower()
